<nav id="adminNavbar">
    ...
    <div class="navbar-search" id="navSearchWrapper">
        ...
        <input
            type="text"
            id="navSearchInput"
            placeholder="Cari modul, pengguna…"
            autocomplete="off"
            hx-get="{{ route('admin.search') }}"
    ...
    <div class="navbar-actions">
        ...
        {{-- Home / back to site --}}
        <a href="{{ route('home') }}" class="navbar-action-btn" title="Lihat Situs">
        ...
        <div class="dropdown">
            ...
            <ul class="dropdown-menu dropdown-menu-end shadow-sm"
                ...
                <li><hr class="dropdown-divider" style="margin:4px 0;border-color:var(--border)"></li>
                <li>
...
{{-- Lightweight search JS (no extra dependency) --}}
<script>
(function () {
    ...
    function renderResults(data, q) {
        if (!data || (!data.moduls?.length && !data.users?.length)) {
            dropdown.innerHTML = `<div class="search-result-item" style="color:var(--text-muted);cursor:default">Tidak ada hasil untuk "<strong>${q}</strong>"</div>`;
        ...
        if (data.moduls?.length) {
            html += `<div style="padding:7px 14px 4px;font-size:10px;font-weight:700;letter-spacing:.6px;text-transform:uppercase;color:var(--text-muted)">Modul</div>`;
        ...
        if (data.users?.length) {
            html += `<div style="padding:7px 14px 4px;font-size:10px;font-weight:700;letter-spacing:.6px;text-transform:uppercase;color:var(--text-muted)">Pengguna</div>`;
