...
@push('styles')
<style>
    .tox-tinymce { border-radius: var(--radius) !important; border-color: var(--border-color) !important; }
    .video-preview { aspect-ratio: 16/9; width: 100%; border-radius: var(--radius); border: 1px solid var(--border-color); background: #000; }
    .resource-item {
...
<div class="page-header d-flex align-items-start justify-content-between flex-wrap gap-2">
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item"><a href="{{ route('admin.moduls.submoduls.show', [$modul, $submodul]) }}">{{ Str::limit($submodul->judul, 20) }}</a></li>
...
<div class="row g-3">
    ...
    <div class="col-lg-8">
        ...
        <form action="{{ route('admin.moduls.submoduls.update', [$modul, $submodul]) }}"
              method="POST" id="editSubmodulForm">
            @csrf
            @method('PUT')
                ...
                <div class="admin-card-body">
                    ...
                    <div>
                        ...
                        <input type="number" name="sort_order" class="form-control"
                               value="{{ old('sort_order', $submodul->sort_order) }}" min="0" style="max-width:120px;">
                        <div class="form-text">Angka kecil tampil lebih awal. Atur urutan visual di Course Builder.</div>
                    </div>
                </div>
            </div>
            ...
            <div class="admin-card mb-3">
                <div class="admin-card-header">
                    ...
                    <span style="font-size:0.78rem;color:#adb5bd;">Gunakan toolbar untuk insert gambar atau embed video</span>
                </div>
                <div class="admin-card-body">
                    @error('konten')
        ...
        <div class="admin-card">
            ...
            <div class="admin-card-body">
                <p style="font-size:0.82rem;color:#adb5bd;margin-bottom:0.75rem;">
                    Tambahkan link YouTube atau Vimeo. Video akan disimpan sebagai resource terpisah dan bisa ditampilkan di halaman submodul.
                </p>
                    ...
                    <div class="d-flex flex-column gap-2 mb-3">
                        ...
                        <div class="resource-item">
                            ...
                            <form action="{{ route('storage.file', $res->path) }}" method="POST"
                                ...
                                {{-- Hapus resource via ResourceController --}}
                                @csrf
                                @method('DELETE')
                                {{-- Karena resource tidak punya dedicated route destroy di admin,
                                     kita pakai endpoint generic jika ada, atau JS fetch --}}
                            </form>
                            <button type="button"
                                    onclick="deleteResource({{ $res->id }}, this)"
                ...
                <form id="addVideoForm">
                    ...
                    <div id="videoPreviewWrap" style="display:none;margin-bottom:0.75rem;">
                        <iframe id="videoPreviewFrame" class="video-preview" frameborder="0"
        ...
        <div class="admin-card">
            ...
            <div class="admin-card-body">
                <dl style="font-size:0.875rem;margin:0;">
                    <dt style="color:#adb5bd;font-size:0.72rem;font-weight:600;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:0.2rem;">Modul</dt>
                    <dd style="margin-bottom:0.75rem;font-weight:500;">{{ $modul->judul }}</dd>

                    <dt style="color:#adb5bd;font-size:0.72rem;font-weight:600;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:0.2rem;">Dibuat</dt>
                    <dd style="margin-bottom:0.75rem;">{{ $submodul->created_at->format('d M Y, H:i') }}</dd>

                    <dt style="color:#adb5bd;font-size:0.72rem;font-weight:600;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:0.2rem;">Terakhir diperbarui</dt>
...
tinymce.init({
    ...
    images_upload_handler: function (blobInfo, progress) {
        return new Promise((resolve, reject) => {
            ...
            .then(data => {
                ...
                else reject({ message: 'Upload gagal', remove: true });
            })
            .catch(() => reject({ message: 'Upload gagal', remove: true }));
        });
    },
    ...
    media_url_resolver: function (data, resolve) {
        ...
        if (embedUrl) {
            resolve({ html: `<iframe src="${embedUrl}" width="100%" height="400" frameborder="0" allowfullscreen></iframe>` });
        } else {
            resolve({ html: '' });
        }
    },
...
<script>
    ...
    document.getElementById('addVideoBtn').addEventListener('click', async function () {
        ...
        try {
            const res = await fetch('{{ route("storage.file", ["path" => "dummy"]) }}'.replace('/dummy', ''), {
                // Kita POST langsung ke ResourceController
                // endpoint: POST /learn/resources (jika ada) atau kita buat endpoint khusus
                // Untuk sementara gunakan endpoint tinymce-style
            });

            // Simpan via fetch ke endpoint custom yang kita tambahkan
